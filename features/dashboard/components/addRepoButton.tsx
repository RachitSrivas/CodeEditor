...
import { importGithubRepo } from "../actions/github"
...
const AddRepo = () => {
  ...
  const handleImport = async (data: { url: string; title: string; template: any }) => {
    try {
      setIsImporting(true);
      const toastId = toast.loading("Importing repository (this may take a minute)...");
      
      const res = await importGithubRepo(data.url, data.template, data.title);
