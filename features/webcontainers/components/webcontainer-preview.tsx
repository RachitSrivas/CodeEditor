...
// Tell Next.js to NEVER render the terminal on the server
    ...
    async function setupContainer() {
      ...
      try {
        ...
        if (hasPackageJson) {
          ...
          // Use npm directly with lockfile optimization and suppressed warnings
          const installArgs = ["install", "--no-audit", "--no-fund", "--legacy-peer-deps", "--loglevel=error"];
          ...
          if (terminalRef.current?.writeToTerminal) {
            terminalRef.current.writeToTerminal("⏭️ No package.json found. Skipping install.\r\n");
          }
        }
        ...
        if (hasPackageJson) {
          try {
            const packageJsonStr = await instance.fs.readFile('package.json', 'utf8');
        ...
        } else if (isLanguageTemplate) {
           // For language templates like Python or C++, starting might be different
           // But normally they don't have a server to start in the same way, or they have a run.js
           startArgs = ["run", "start"];
        }
    ...
    <div className="h-full w-full flex flex-col">
      {!previewUrl ? (
        ...
        <div className="h-full flex flex-col">
          ...
          <div className={isLanguageTemplate ? "flex-1 p-4" : "h-64 border-t"}>
            <TerminalComponent 
              ref={terminalRef}
              webContainerInstance={instance}
              theme="dark"
