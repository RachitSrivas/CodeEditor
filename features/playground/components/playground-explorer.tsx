...
import { SiHtml5, SiCss, SiJavascript, SiTypescript, SiReact, SiJson, SiMarkdown } from "react-icons/si";

function getFileIcon(filename: string, extension: string) {
  const ext = extension.toLowerCase();
  switch (ext) {
    case 'html': return <SiHtml5 className="h-4 w-4 mr-2 shrink-0 text-orange-500" />;
    case 'css': return <SiCss className="h-4 w-4 mr-2 shrink-0 text-blue-500" />;
    case 'js': return <SiJavascript className="h-4 w-4 mr-2 shrink-0 text-yellow-400" />;
    case 'ts': return <SiTypescript className="h-4 w-4 mr-2 shrink-0 text-blue-400" />;
    case 'jsx': 
    case 'tsx': return <SiReact className="h-4 w-4 mr-2 shrink-0 text-blue-400" />;
    case 'json': return <SiJson className="h-4 w-4 mr-2 shrink-0 text-yellow-500" />;
    case 'md': return <SiMarkdown className="h-4 w-4 mr-2 shrink-0 text-slate-500" />;
    default: return <File className="h-4 w-4 mr-2 shrink-0 text-muted-foreground" />;
  }
}
...
function TemplateNode({
  ...
  if (!isFolder) {
    ...
    if (isLanguageTemplate && (fileName === "package.json" || fileName === "run.js")) {
      return null
    }
      ...
      <SidebarMenuItem>
        <div className="flex items-center group">
          <SidebarMenuButton isActive={isSelected} onClick={() => onFileSelect?.(file)} className="flex-1">
            {getFileIcon(file.filename, file.fileExtension)}
            <span>{fileName}</span>
          </SidebarMenuButton>
